...
function samplePanelFromPool(personas, n = 4, anchorPersona = null, country = 'KR') {
  ...
  return picked.map((p, idx) => {
    ...
    return {
      ...
      _isAnchor: !!p._isAnchor, // CEO 2026-06-24: 프론트 대화기록 key 통일용 앵커 플래그 명시 전달
...
페르소나가 **남성**일 때:
- 보이그룹/남자 아이돌/남자 배우 → "오빠"가 아니라 "(그룹명) 멤버", "(이름)", "형", "(이름)씨" 사용
- 걸그룹/여자 아이돌/여자 배우 → "누나"가 아니라 "(그룹명) 멤버", "(이름)", "(이름)씨" 사용
- "오빠/언니" 같은 여성 화자 특유의 1인칭 호칭 절대 금지
페르소나가 **여성**일 때 걸그룹은 "(이름)", "언니", "(이름)언니" 가능, 보이그룹은 "오빠" 가능.
연령에 따른 위계도 반영: 페르소나가 30대 이상 남성이라면 20대 아이돌을 "친구/동생/(이름)"로 부르는 게 자연스럽습니다.

[narratives/quote/values 작성 시 1인칭 화자 어조]
- who/life/mind/love/buy 단락 안에 인용/생각/말투가 들어갈 때 페르소나 성별·연령에 맞는 1인칭 어조 사용
- quote 필드: 페르소나가 직접 말하는 한 줄 — 성별·연령에 맞는 호칭/어말어미
...
mediaHabits는 위 채널 점유율과 일치해야 하며, ${meta.name}에서 차단된 매체는 제외하세요.`;
  ...
  try {
    ...
    // CEO 2026-06-12: 각 패널 멤버에 unique id 부여 — 프론트 캐시 공있용
    let _panelArr = result.json.panel;
    // CEO 2026-06-24: anchorPersona 있으면 panel[0]에 단일 페르소나(이름 그대로) 강제 고정 +
    //   _isAnchor 플래그 — 단일↔패널 앵커 대화기록 key 통일(휘발 방지). LLM이 만든 첫 멤버는 1칸 밀어 (n-1)명으로.
    if (anchorPersona && anchorPersona.name) {
      const _anchorMember = {
        name: anchorPersona.name,
        age: anchorPersona.age || null,
        gender: anchorPersona.gender || null,
        occupation: anchorPersona.occupation || null,
        archetype: anchorPersona.archetype || (_panelArr[0] && _panelArr[0].archetype) || '',
        quote: anchorPersona.quote || (_panelArr[0] && _panelArr[0].quote) || '',
        lifestyle: anchorPersona.lifestyle || (_panelArr[0] && _panelArr[0].lifestyle) || '',
        values: anchorPersona.values || (_panelArr[0] && _panelArr[0].values) || [],
        mediaHabits: anchorPersona.mediaHabits || (_panelArr[0] && _panelArr[0].mediaHabits) || [],
        purchaseDrivers: anchorPersona.purchaseDrivers || (_panelArr[0] && _panelArr[0].purchaseDrivers) || [],
        painPoints: anchorPersona.painPoints || (_panelArr[0] && _panelArr[0].painPoints) || [],
        _isAnchor: true,
      };
      // 앵커를 맨 앞에 두고 나머지는 LLM 생성분으로 n명 유지
      _panelArr = [_anchorMember, ..._panelArr.filter(m => (m.name || '') !== anchorPersona.name)].slice(0, n);
    }
    const panelWithIds = _panelArr.map((p, i) => ({
      ...p,
      id: p._isAnchor ? `panel-anchor-${Date.now()}-${(p.name||'').replace(/\s+/g,'').substring(0,8)}`
                      : `panel-${Date.now()}-${i}-${(p.name||'').replace(/\s+/g,'').substring(0,8)}`,
    }));
    res.json({
      ok: true,
      country: meta,
      ...
      meta: { method: "gemini-2.5-flash", count: panelWithIds.length },
    });
  } catch (e) {
    res.json({
      ok: true,
...
const REALTIME_KEYWORDS = [
  // 시점
  "최근", "요즘", "오늘", "이번", "이번 주", "이번달", "이번 달", "최신",
  "동향", "근황", "트렌드", "이슈", "뉴스", "화제", "논란", "바이럴",
  "신제품", "신곡", "수상", "은퇴", "광고", "캠페인", "컴백",
  // 취향·토픽 (페르소나가 답할 때 옛 정보로 답하면 안 되는 영역)
  "음악", "노래", "가수", "아이돌", "그룹", "콘서트", "앨범",
  "드라마", "영화", "OTT", "넷플", "티빙", "쿠팡플", "디즈니",
  "게임", "콘텐츠", "유튜브", "쇼츠", "릴스", "스트리머",
  "패션", "브랜드", "스타일", "코디",
  "뷰티", "화장품", "스킨케어", "메이크업",
  "맛집", "음식", "레시피", "카페", "디저트",
  "여행", "여행지", "휴가",
...
[질문 이해와 정확성 규칙 — 절대]
· **질문을 정확히 읽고** 지금 묻는 것이 무엇인지 먼저 파악하세요. 다른 주제로 흐르지 마세요.
  예: "비싸도 사고 싶은 브랜드"를 물으면 반드시 구매·브랜드 이야기만 하세요. 이전 대화 주제(예: 음악)로 돌아가지 마세요.
· **확실하지 않은 사실은 아는 척 하지 마세요.** 일부러 디테일을 만들지 마세요(멤버명, 곡제목, 회사명 등).
  - 알고 있다면 본인의 취향으로 이야기하세요.
  - 잘 모르면 "아 잘 모르겠는데… 그런 멤버가 있었나요?" 식으로 솔직히 인정하세요.
  - K-Pop 그룹의 멤버, 가수, 노래, 콘텐츠 등은 실제 존재하는 것만 제대로 이야기하세요.
· **사용자가 지적하면 인정**하고 대화를 이어나가세요. 회피하거나 끝에서 "…"만 찍지 마세요.

[아티스트·그룹명 자동 보정 절대 금지 — 가장 중요]
· 사용자가 말한 아티스트·그룹·인물·곡명을 **단 한 글자도 임의로 바꾸지 마세요**. 비슷한 이름과 자동 매칭하지 마세요.
  - 잘못된 예: 사용자가 "리센느"라고 했는데 "르세라핌"으로 바꿔서 답하기 → 절대 금지
  - 올바른 예: "‘리센느’요? 죄송한데 잘 모르는 그룹이에요. 어떤 그룹인지 알려주세요ㅠㅠ"
  - 모르는 이름은 "모른다"가 정답입니다. 추측해서 비슷한 이름으로 답하지 마세요.
· **모든 토픽**에서 패널 멤버가 똑같은 답을 하면 안 됩니다. 나이·세대·성별·직업·소득·관심사에 따라 서로 다른 구체적 소재를 이야기하세요. 한 단어로 끝내지 말고 본인 취향·이유·경험을 1–2문장으로 함께 말하세요.
· **옛 트렌드·옛 인물·옛 작품 금지** — 모델이 학습한 과거 데이터에 의존하지 마세요. 위의 [실시간 웹 검색 요약]이 있으면 그 내용을 우선 사용하고, 없으면 본인이 모른다고 솔직히 답하거나 일반적이고 시점에 둔감한 소재(예: "노래는 그때그때 차트 보면서 들어요", "드라마는 OTT에서 추천해주는 거 봐요")로 답하세요.
· **자동 보정 금지** (다시 강조): 사용자가 말한 아티스트·작품·브랜드명을 모르면 모른다고 답하세요. 비슷한 이름과 자동 매칭하지 마세요.
