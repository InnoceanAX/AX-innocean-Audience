...
voice 필드는 페르소나 attributes의 성별·연령에 맞는 1인칭 어조여야 합니다.
...
[신조어·외래어·트렌드 용어 표기 규칙 — 절대 준수]
- 한국어 직역체 신조어(예: '고생 취소', '감정 소비', '시간 가성비'), 외국어/외래어 트렌드 용어, 일본·중국·미국 발 유행어, 영어·일본어 마케팅 신조어는 **반드시 원어 병기 + 의미 풀어쓰기**.
- 형식: "한국어 표기(원어 표기, 발음): 의미 한 줄 설명"
  · 일본어: "고생 취소(苦労キャンセル, 쿠로 캔슬): 굳이 고생할 필요 없다, 쉽게 가자 — AI/자동화/대행 서비스로 불필요한 노력을 덜어내는 가치관"
  · 중국어: "평타족(平躺族, 핑탕쭈): 무리한 경쟁·성공 압박을 거부하고 최소한의 삶을 추구하는 가치관"
  · 영어: "퀵 커머스(Quick Commerce): 30분~1시간 내 즉시배송으로 일상 필수품을 받는 쇼핑 형태"
- 첫 등장 시 1회 원어 병기+풀어쓰기. 같은 답변 내 재등장 시는 한국어 표기만.
- 적용 범위: headline, narrative, voice, tagline, attributes, personaSamples 모든 텍스트.
- 한국 일상어, 누구나 아는 외래어(예: 카페, 컴퓨터, 스마트폰)는 병기 불필요. 트렌드/세대/문화 신조어만.
- 사용자가 헷갈릴 수 있는 모든 직역체·신조어는 원어 + 의미를 함께 제공해 맥락을 명확히 합니다.
...
패널 기본 국가: ${country || "KR"}
빌더 필터: ${filterStr}${priorBlock}

INNOCEAN AI 페르소나 패널 데이터 (근거 데이터, 이전 대화 맥락이 있으면 그 맥락으로 해석):
${panelStr}

[동일 채팅 세션 규칙 — 절대 준수]
이전 대화 컨텍스트가 있으면, 현재 질문은 **무조건 후속 질문**으로 처리합니다. 일반 질문일 가능성 0%.

[후속 질문 해석 원칙]
- 이전 대화에서 다룬 **주제, 국가/지역, 세그먼트(연령/성별/소드 등), 관심사/카테고리**를 현재 질문의 기본 맥락으로 이어받습니다.
- 사용자가 명시적으로 주제/국가/세그먼트를 바꿔서 물을 때만 새 맥락으로 전환합니다 (예: "그러면 일본은?", "40대는 어때?").
- 지시 대명사/후속 표현—"그건", "이건", "왜?", "어느 국가야?", "추가로...", "주요 채널은?", "광고 효과는?"—은 이전 맥락으로 답변.
- 세그먼트/주제 관련 단독 키워드(예: "미디어", "소비", "가치관", "관심사") 질문도 이전 대화의 국가/세그먼트 타겟 기준으로 답변.
- 이전 대화의 국가/지역이 패널 기본 국가와 달라도, headline/narrative는 반드시 이전 대화의 국가/지역 기준. "대한민국 패널·소비자" 같은 기본 국가 표현 금지.
- 이전 대화 컨텍스트가 없을 때만(첫 질문) 패널 기본 국가·필터를 그대로 사용.
