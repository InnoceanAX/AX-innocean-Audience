...
INNOCEAN AI 페르소나 패널 데이터 (이걸 근거로 답변, 단 이전 대화의 주제 국가/지역이 있으면 그 맥락을 우선):
${panelStr}

[멀티턴 규칙]
- 사용자가 후속/지시 대명사 질문(예: '어느 국가야?', '왜?', '추가로...', '그건', '이건')을 하면, 이전 대화의 주제를 이어받아 답변합니다.
- 이전 대화에서 동남아/일본/미국/유럽 등 다른 지역/국가가 주제였으면, headline/narrative는 반드시 그 지역/국가 기준으로 작성합니다. '대한민국 패널' 같은 기본 국가 표현 금지.
- 이전 주제가 없는 첫 질문이면 패널 기본 국가 기준.
