...
import {
  ...
  listAudienceSources, listBaselineCountries, getAgeSeniorShare, getAgeYouthShare,
...
audienceRouter.get("/sources", async (req, res) => {
  const baselineCount = listBaselineCountries().length;
  res.json({
    ok: true,
    sources: listAudienceSources(),
    coverage: `${baselineCount} countries (베이스라인 적재 기준)`,
...
audienceRouter.get("/compare-all", async (req, res) => {
  ...
  const who = supported.map(r => ({
    ...
    pop60: getAgeSeniorShare(r.who.ageBuckets),
    pop15_29: getAgeYouthShare(r.who.ageBuckets),
  }));
...
[요약(summary) 작성 규칙 - 필수]
- 각 차원(who/life/mind/love/buy)의 summary는 빌더 타겟 기준의 분석 결과를 서술합니다.
- 베이스라인(국가 평균) 데이터를 직접 나열하지 마세요. '국가 평균 대비 +N%' 같은 비교 형태로만 사용하세요.
- 예시 좋은 summary: '타겟 오디언스의 75%가 대도시 거주자로, 국가 평균(65%) 대비 10%p 높습니다.'
- 예시 나쁜 summary: '한국의 도시화율은 81.6%이며 중위연령은 45.6세입니다.' (베이스라인 나열)
- summary는 3문장 이내로 간결하게, 타겟의 특징과 시사점 중심으로 작성합니다.`;
